...
export class BrandService extends BaseRepository {
  ...
  async create(data: { name: string; logo_url?: string | null; description?: string; is_primary?: boolean }): Promise<Brand> {
  ...
  async update(id: string, data: Partial<{ name: string; logo_url: string | null; description: string; is_primary: boolean }>): Promise<Brand> {
